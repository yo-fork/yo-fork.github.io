(() => {
  ...
  function addMainTab() {
    ...
    if (!nav || !mdButton || $('btn-json')) return;

    const jsonButton = mdButton.cloneNode(false);
    jsonButton.id = 'btn-json';
    jsonButton.textContent = 'JSON';
    jsonButton.className = 'px-4 py-3 flex-1 font-bold hover:bg-slate-700';
  ...
  function addJsonSection() {
    ...
    const modeRow = createEl('div', { class: 'grid grid-cols-3 gap-1 shrink-0' }, [
    ...
    const tools = createEl('details', { id: 'json-tools', class: 'tool-details shrink-0' }, [
      ...
      createEl('label', { class: 'json-option-row mt-2' }, [
        createEl('input', { id: 'json-flatten', type: 'checkbox', checked: '' }),
        document.createTextNode(' Flatten nested objects')
      ])
    ]);

    const stats = createEl('div', { id: 'json-stats', class: 'shrink-0 rounded bg-white border border-slate-200 px-2 py-1 text-xs text-slate-600', text: 'JSON/JSONLを貼り付けてください' });
    const status = createEl('div', { id: 'json-status', class: 'hidden status shrink-0' });
    ...
    const tableWrap = createEl('div', { id: 'json-table-wrap', class: 'json-table-wrap' });
    shell.append(tableWrap);
    workspace.append(input, shell);

    section.append(modeRow, tools, stats, status, workspace);
  ...
  function setupStyles() {
    ...
    style.textContent = `
      ...
      .json-table-wrap { min-width:100%; overflow:auto; }
      .json-table { border-collapse: separate; border-spacing: 0; width: 100%; font-size: .84rem; color: #172033; }
      .json-table th, .json-table td { border-bottom: 1px solid #e2e8f0; border-right: 1px solid #edf2f7; padding: .45rem .55rem; vertical-align: top; white-space: nowrap; max-width: 22rem; overflow: hidden; text-overflow: ellipsis; }
      ...
      .json-table td:first-child, .json-table th:first-child { position: sticky; left: 0; z-index: 2; background: #ffffff; }
      ...
      .json-error { color: #b91c1c; background: #fef2f2; border: 1px solid #fecaca; padding: .75rem; border-radius: .75rem; white-space: pre-wrap; }
    `;
  ...
  function parseInput(text) {
    ...
    try {
      ...
      if (errors.length) {
        throw new Error('JSONとしてもJSONLとしてもパースできません。\n\n' + errors.slice(0, 8).join('\n'));
      }
  ...
  function normalizeRows(value) {
    ...
    if (value && typeof value === 'object') {
      ...
      const arrayKey = keys.find((key) => Array.isArray(value[key]));
  ...
  function flattenObject(value, prefix = '', out = {}) {
    ...
    if (Array.isArray(value)) {
      out[prefix || 'value'] = value.map((item) => stringifyCell(item)).join(', ');
      return out;
    }
    const keys = Object.keys(value);
    if (keys.length === 0) {
    ...
    keys.forEach((key) => {
      ...
      else if (Array.isArray(child)) out[path] = child.map((item) => stringifyCell(item)).join(', ');
      else out[path] = child;
    });
    return out;
  }
  ...
  function renderTable() {
    ...
    const flatten = $('json-flatten').checked;
    if (!input || !wrap || !stats) return;
    ...
    try {
      ...
      const { objects, columns } = makeTableRows(rows, flatten);
      if (objects.length === 0) {
      ...
      const pre = createEl('div', { class: 'json-error', text: error.message || String(error) });
      wrap.append(pre);
    }
  }
  ...
  function buildCsv() {
    ...
    objects.forEach((row) => lines.push(columns.map((column) => csvEscape(row[column])).join(',')));
    return lines.join('\n');
  }
  ...
  function setJsonMode(mode) {
    ...
    const buttons = {
      edit: $('json-mode-edit'),
      split: $('json-mode-split'),
      table: $('json-mode-table')
    };
    ...
    localStorage.setItem('my-playground-json-mode-v1', mode);
  ...
  function boot() {
    ...
    input.style.fontSize = localStorage.getItem('my-playground-json-font-size-v1') || '16px';
    ...
    $('json-sample').addEventListener('click', () => { input.value = sampleJson(); renderTable(); notify('サンプルを読み込みました'); });
    $('json-clear').addEventListener('click', () => { if (confirm('JSON入力を空にしますか？')) { input.value = ''; renderTable(); } });
    ...
    $('json-export-csv').addEventListener('click', () => {
      try {
        downloadText('data.csv', buildCsv(), 'text/csv;charset=utf-8');
        notify('CSVを書き出しました');
      } catch (error) {
        renderTable();
      }
    ...
    $('json-font-smaller').addEventListener('click', () => {
      ...
      localStorage.setItem('my-playground-json-font-size-v1', input.style.fontSize);
    });
    ...
    const mode = localStorage.getItem('my-playground-json-mode-v1') || 'edit';
    setJsonMode(mode);
  }
